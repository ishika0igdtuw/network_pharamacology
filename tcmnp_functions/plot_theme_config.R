...
PLOT_THEME_CONFIG <- list(
    ...
    font_sizes = list(
        base = 14, # Base font size for all text
        title = 18, # Plot titles
        subtitle = 14, # Subtitles
        axis_title = 14, # Axis titles
        axis_text = 12, # Axis labels/ticks
        legend_title = 13, # Legend title
        legend_text = 11, # Legend labels
        label = 10, # Node/point labels
        small = 9 # Annotations, captions
    ),

    # Font families (consistent across all plots)
    fonts = list(
        primary = "Helvetica", # Main font (Helvetica/Arial)
    ...
    # Text styling
    text = list(
        title_face = "bold", # Title font face
        title_align = 0.5, # Center alignment
        label_case = "sentence", # "sentence", "upper", "lower", "title"
    ...
    output = list(
        ...
        formats = c("png", "pdf"), # Export formats
...
#' Calculate canvas size based on data complexity
...
calculate_canvas_size <- function(n_elements,
                                  base_width = NULL,
                                  base_height = NULL) {
    if (is.null(base_width)) {
        base_width <- PLOT_THEME_CONFIG$output$min_width
    }
    if (is.null(base_height)) {
        base_height <- PLOT_THEME_CONFIG$output$min_height
    }

    # Scale based on number of elements
    scale <- PLOT_THEME_CONFIG$output$scale_factor

    if (n_elements > 50) {
        width <- base_width * scale
        height <- base_height * scale
    } else if (n_elements > 100) {
        width <- base_width * scale * 1.3
        height <- base_height * scale * 1.3
    } else {
        width <- base_width
        height <- base_height
    }
    ...
    # Apply limits
    width <- min(
        max(width, PLOT_THEME_CONFIG$output$min_width),
        PLOT_THEME_CONFIG$output$max_width
    )
    height <- min(
        max(height, PLOT_THEME_CONFIG$output$min_height),
        PLOT_THEME_CONFIG$output$max_height
    )
