integrated_np_disease_network <- function(tcm_data, out_dir = "outputs", disease_label = "Breast Cancer") {
    ...
    common_targets_path <- file.path("outputs", "disease_target_overlap", "common_targets.csv")

    if (!file.exists(common_targets_path)) {
        stop(paste("Error: Common targets file not found at:", common_targets_path))
    }

    common_targets_df <- read_csv(common_targets_path, show_col_types = FALSE)
    disease_targets_list <- common_targets_df$common_target
