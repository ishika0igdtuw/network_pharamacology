...
export_for_cytoscape <- function(network.data, out_dir, file_prefix = "network") {
  ...
  if (all(c("herb", "molecule", "target") %in% names(network.data))) {
    ...
    e1 <- network.data %>%
      dplyr::select(Source = herb, Target = molecule) %>%
      mutate(Interaction = "contains", Type = "Herb-Molecule")
    e2 <- network.data %>%
      dplyr::select(Source = molecule, Target = target) %>%
      mutate(Interaction = "targets", Type = "Molecule-Target")
  ...
  sif_data <- edges %>% dplyr::select(Source, Interaction, Target)
