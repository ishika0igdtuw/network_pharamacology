...
disease_target_overlap <- function(
  ...
  for (efo_id in disease_ids) {
    ...
    if (status_code(res) == 200) {
      ...
      d_targets <- sapply(rows, function(x) x$target$approvedSymbol) |> unlist() |> unique()
  ...
  for (name in names(target_lists)) {
    ...
    write.csv(data.frame(symbol = target_lists[[name]]), 
              file.path(out_dir, paste0("set_", safe_name, ".csv")), row.names = FALSE)
  }
  ...
  if (n_sets <= 3) {
    p_venn <- ggVennDiagram(target_lists, label_alpha = 0) +
      ...
      labs(title = "Disease-Target Overlap Analysis",
  ...
  if (n_sets > 1) {
    ...
    write.csv(data.frame(symbol = common_all), 
              file.path(out_dir, "common_intersection_all.csv"), row.names = FALSE)
    ...
    write.csv(data.frame(symbol = our_unique), 
              file.path(out_dir, "predicted_unique_targets.csv"), row.names = FALSE)
  }
